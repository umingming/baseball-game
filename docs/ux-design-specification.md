...
| 계층 | 용도 | 비주얼 | 예시 |
|------|------|--------|------|
| **Primary (주요)** | 현재 화면의 핵심 액션. 화면당 1개 | 팀 컬러 배경 + 화이트 텍스트. 가장 큰 크기(48px 높이) | "드래프트 확정", "시설 해금", "이어서 플레이" |
| **Secondary (보조)** | 대안 선택지, 부가 액션 | 투명 배경 + 팀 컬러 테두리 + 텍스트. 중간 크기(40px) | "취소", "나중에", "상세 보기" |
| **Tertiary (약한)** | 부차적 액션, 링크형 | 텍스트만. 밑줄 또는 색상 구분. 최소 크기(32px) | "건너뛰기", "더 보기", "닫기" |

**긴급 팝업 선택지 규칙:**
- 선택지 간 시각적 무게가 동등해야 함 (어느 하나로 유도하지 않음)
- 모든 선택지 동일한 크기의 Secondary 스타일
- "감독에게 위임" 같은 안전한 선택지가 항상 마지막에 위치

**터치 피드백:**
- 모든 버튼: 탭 시 2px 아래로 눌림 + 밝기 10% 감소 (1프레임)
- 비활성 버튼: 50% 불투명도 + 탭 무반응
- 중요 액션 완료 후: 짧은 진동 피드백 (햅틱, 설정에서 끄기 가능)
...
| 유형 | 시각 | 위치 | 지속 시간 | 사운드 |
|------|------|------|:---------:|:------:|
| **성공/긍정** | 초록 배경 배너 + ▲ 아이콘 | 화면 상단 슬라이드 인 | 3초 자동 닫힘 | 짧은 종소리 |
| **업적 해금** | 골드 테두리 배너 + 트로피 아이콘 | 화면 상단 중앙 팝 | 4초 자동 닫힘 (탭 시 컬렉션 이동) | 팡파르 효과음 |
| **경고/주의** | 노랑 배경 배너 + ━ 아이콘 | 화면 상단 슬라이드 인 | 유저 닫기까지 유지 | 없음 |
| **위험/긴급** | 빨강 테두리 팝업 + 화면 어둡게 | 화면 중앙 | 유저 응답까지 유지 | 경고 효과음 |
| **정보** | 하늘색 배경 배너 + ⓘ 아이콘 | 화면 상단 슬라이드 인 | 3초 자동 닫힘 | 없음 |
| **티커 이벤트** | 티커 바 내 텍스트 색상 변화 | 하단 티커 고정 | 다음 이벤트까지 | 홈런 등 주요 이벤트만 |

**피드백 우선순위 (동시 발생 시):**
1. 긴급 팝업 (최우선, 다른 피드백 대기)
2. 업적 해금 배너 (긴급 팝업 닫힌 후 표시)
3. 성공/경고/정보 배너 (큐에 쌓여 순차 표시)
4. 티커 이벤트 (항상 독립적으로 표시)
...
| 상황 | 동작 |
|------|------|
| 메뉴 항목 → 오버레이 패널 기능 (선수단, 재정, 뉴스) | 메뉴 닫힘 → 오버레이 패널 올라옴. 구장 배경 유지 |
| 메뉴 항목 → 전체 화면 기능 (컬렉션, 순위표) | 메뉴 닫힘 → 전체 화면 전환. 뒤로가기로 구장 복귀 |
| 메뉴 항목 → 구장 (이미 메인) | 메뉴 닫힘. 오버레이 패널도 닫힘 |
| 긴급 팝업 발생 시 | 사이드 메뉴/오버레이 패널 위에 팝업 표시. 메뉴는 닫히지 않음 |

**뒤로가기 계층:**
1. 팝업 열림 → 팝업 닫기
2. 오버레이 패널 열림 → 패널 닫기
3. 전체 화면 (컬렉션 등) → 구장 메인으로 복귀
4. 사이드 메뉴 열림 → 메뉴 닫기
5. 구장 메인 → 앱 종료 확인 팝업

**화면 전환 방식:**
- 사이드 메뉴: 좌측에서 슬라이드 인/아웃 (300ms)
- 오버레이 패널: 하단에서 슬라이드 업/다운 (250ms)
- 전체 화면 전환: 페이드 (200ms)
- 팝업: 스케일 팝 (150ms)
...
| 상태 | 색상 | 형태 (색약 대응) | 적용 대상 |
|------|------|:---------------:|----------|
| 좋음/양호 | 초록 | ▲ 위쪽 화살표, 웃는 표정 | 선수 컨디션, 재정 흑자, 팀 분위기 |
| 보통/주의 | 노랑 | ━ 수평선, 보통 표정 | 선수 피로, 재정 적정, 약간의 불화 |
| 나쁨/위험 | 빨강 | ▼ 아래 화살표, 찡그린 표정 | 선수 부상, 재정 적자, 팀 위기 |
| 특별/희귀 | 금색 | ★ 별, 반짝임 | S등급, MVP, 희귀 업적 |
| 미확인/잠김 | 회색 | 🔒 자물쇠, ??? 텍스트 | ???선수, 미해금 시설/업적 |

**등급 표현:**

| 등급 | 색상 | 크기/강조 |
|------|------|----------|
| S | 금색 + 반짝임 애니메이션 | 1.5배 크기, 글로우 효과 |
| A | 밝은 초록 | 기본 크기, 볼드 |
| B | 하늘색 | 기본 크기 |
| C | 노랑 | 기본 크기 |
| D | 빨강 | 기본 크기 |
...
| 상황 | 표현 |
|------|------|
| 컬렉션 첫 진입 (업적 0개) | 빈 그리드 + "아직 해금된 업적이 없습니다. 경기를 진행하면 숨겨진 업적이 해금됩니다!" |
| 뉴스 없음 | "오늘의 소식이 없습니다. 경기가 시작되면 뉴스가 업데이트됩니다" |
| 선수단 빈 포지션 | 실루엣 + "공석" 라벨 + "드래프트/FA에서 영입하세요" |
| 시설 전부 잠김 (초기) | 실루엣 그리드 + "자금과 업적을 모아 시설을 해금하세요!" |

**로딩 상태:**
- 게임 로드: 픽셀 야구공이 통통 튀는 애니메이션 + 로딩 팁(게임 운영 힌트 랜덤 표시)
- 세이브: 화면 우상단에 작은 저장 아이콘 회전 (2초). 유저 인터랙션 차단 없음
- 시즌 전환: "다음 시즌 준비 중..." + 구장 변화 프리뷰 애니메이션
...
| 원칙 | 설명 |
|------|------|
| **픽셀 충실** | 모든 애니메이션은 프레임 기반. 서브픽셀 이동 없음. 정수 배율만 사용 |
| **짧고 명확** | UI 전환 150~300ms. 보상 연출 최대 2초. 스킵 불가 연출 없음 |
| **의미 있는 움직임만** | 장식적 애니메이션 최소화. 피드백/보상/전환에만 사용 |
| **배터리 의식** | 아이소메트릭 구장의 캐릭터 움직임은 저전력 모드에서 정적 이미지로 대체 |

**핵심 연출 목록:**

| 연출 | 타이밍 | 스킵 가능 |
|------|--------|:---------:|
| 구단 창단 | 1.5초 (로고 등장 + 팡파르) | ❌ (1회만) |
| 드래프트 선수 선택 | 1초 (카드 뒤집기 + 능력 공개) | ❌ |
| 시설 해금 | 2초 (실루엣→건물 전환 + 건설 효과) | ❌ |
| 업적 해금 배너 | 0.5초 팝 + 4초 표시 | ◯ (탭으로 닫기) |
| 시즌 리포트 등급 공개 | 1.5초 (카운트업 + 등급 등장) | ◯ |
| 경기 결과 | 0.5초 (스코어 강조) | ◯ |
| 긴급 팝업 등장 | 0.3초 (어둡게 + 팝업 스케일) | ❌ |
...
이 게임은 모바일(세로 모드) 전용이며 데스크톱/태블릿 대응이 불필요하다. 다양한 모바일 화면 비율과 크기에 대응하는 적응형 레이아웃을 설계한다.

| 대응 범위 | 사양 |
|----------|------|
| **최소 화면** | 720×1280 (HD, 16:9) |
| **기준 화면** | 1080×1920 (FHD, 16:9) |
| **최대 비율** | 1080×2400 (FHD+, 20:9) |
| **방향** | 세로 모드 고정 (가로 모드 미지원) |

**적응형 레이아웃 규칙:**

| 요소 | 고정/가변 | 적응 방식 |
|------|:---------:|----------|
| 상태 바 | 고정 24px | 노치/펀치홀 영역은 시스템 상태바로 흡수. 게임 상태바는 세이프 에어리어 내부 |
| 메인 영역 (구장) | 가변 | 화면 비율에 따라 세로 확장. 20:9 폰에서 구장이 더 넓게 보임 |
| 티커 바 | 고정 56px | 하단 고정. 네비게이션 바/제스처 바 영역 위에 배치 |
| 사이드 메뉴 | 고정 폭 70% | 화면 너비의 70% (최대 240px). 나머지 30%에 어두운 오버레이 |
| 오버레이 패널 | 가변 높이 | 최소 40% ~ 최대 70% 화면 높이. 내용에 따라 조절 |
| 팝업 | 고정 너비 85% | 화면 너비의 85% (최대 280px). 항상 중앙 정렬 |

**픽셀 아트 스케일링:**

| 규칙 | 설명 |
|------|------|
| 정수 배율만 사용 | 2x, 3x, 4x 스케일링. 소수점 배율 금지 (픽셀 뭉개짐 방지) |
| 기준 해상도 | 게임 내부 렌더링 360×640 (또는 270×480) → 디바이스 해상도에 정수배로 업스케일 |
| 여백 처리 | 비율 불일치 시 레터박스 대신, 메인 영역(구장)을 확장하여 채움 |
| UI 요소 | dp 단위로 정의 후 디바이스 density에 맞춰 렌더링. 터치 영역은 물리적 48dp 보장 |
...
| 비율 | 대표 기기 | 레이아웃 조정 |
|------|----------|-------------|
| **16:9** | 구형 기기 | 메인 영역 기본 비율. 모든 UI 요소 기본 배치 |
| **18:9** | 대부분의 현세대 폰 | 메인 영역 세로 확장. 구장 뷰에 더 많은 시설 표시 가능 |
| **19.5:9** | iPhone 시리즈 | 상단 노치 영역 회피. 하단 홈 인디케이터 영역 확보 |
| **20:9** | 최신 Android | 메인 영역 최대 확장. 오버레이 패널 높이 여유 확보 |

**세이프 에어리어 처리:**
- **상단:** 시스템 상태바 + 노치/펀치홀 영역을 OS에서 제공하는 세이프 에어리어 인셋으로 처리. 게임 UI는 인셋 아래부터 시작
- **하단:** Android 네비게이션 바 / iOS 홈 인디케이터 영역 위에 티커 바 배치. 제스처 네비게이션 충돌 방지
- **좌우:** 커브드 디스플레이 대응. UI 요소는 좌우 양쪽에 최소 8dp 안전 마진

### Accessibility Strategy

**접근성 목표:** 게임 접근성 가이드라인 (IGDA Game Accessibility Guidelines) 기본 레벨 준수

**시각 접근성:**

| 항목 | 기준 | 구현 방법 |
|------|------|----------|
| 색상 대비 | 텍스트-배경 4.5:1 이상 | 픽셀 아트는 안티앨리어싱이 없어 자연스럽게 높은 대비. 디자인 토큰에서 대비 보장 |
| 색약 대응 | 색상 단독으로 정보 전달 금지 | 모든 상태 표시에 색상+형태 이중 인코딩 |
| 텍스트 크기 | 본문 최소 12sp | 시스템 접근성 폰트 크기 설정 연동 검토 (1.5배까지) |
| 깜빡임 | 초당 3회 미만 | 업적 해금, 긴급 팝업 반짝임 효과 제한 |
| 밝기/다크모드 | 기본이 다크 UI | 밝은 환경 대응: UI 패널 불투명도 조절 옵션 |

**조작 접근성:**

| 항목 | 기준 | 구현 방법 |
|------|------|----------|
| 터치 영역 | 최소 48×48dp | 모든 인터랙티브 요소에 적용 |
| 한 손 조작 | 엄지 도달 범위 내 핵심 액션 | 핵심 버튼은 화면 하단 2/3 이내 |
| 시간 압박 없음 | 타이머 기반 실패 없음 | 긴급 팝업은 분위기 연출만. 무제한 대기 가능 |
| 제스처 대안 | 스와이프 필수 인터랙션에 버튼 대안 | 카드 덱 스와이프 → 좌우 화살표 버튼 병행 |
| 실수 방지 | 중요 결정 전 확인 단계 | 시설 해금, FA 계약 등 되돌릴 수 없는 액션에 확인 팝업 |

**인지 접근성:**

| 항목 | 기준 | 구현 방법 |
|------|------|----------|
| 정보 단순화 | 한 화면 한 판단 | 오버레이 패널에 핵심 정보만. 상세는 탭하면 표시 |
| 단계적 복잡도 | 초기 기능 제한 | 1시즌은 핵심 기능만 노출. 2~3시즌에 걸쳐 점진 해금 |
| 명확한 피드백 | 액션 결과 즉시 전달 | 모든 탭에 시각+음향 피드백. 결과를 모호하게 두지 않음 |
| 복귀 용이 | 이탈 후 빠른 상황 파악 | 복귀 대시보드 (3초 요약). 시즌 타임라인 |
| 읽기 부담 | 긴 텍스트 최소화 | 뉴스/기사는 헤드라인 + 1~2줄 요약. NPC 대화 없음 |

**사운드 접근성:**
- 모든 음향 피드백에 시각적 대안 존재 (사운드 없이 플레이 가능)
- 효과음 ON/OFF 독립 설정 (BGM / 효과음 / 진동 각각 분리)
- 긴급 팝업: 사운드 + 화면 어둡게 + 진동의 3중 알림 (하나만으로도 인지 가능)
...
| 우선순위 | 기기 | 이유 |
|:--------:|------|------|
| P0 | Galaxy S23 (6.1", 19.5:9) | Android 기준 기기, 타겟 유저 최다 보유 |
| P0 | iPhone 14 (6.1", 19.5:9) | iOS 기준 기기 (Post-MVP) |
| P1 | Galaxy A14 (6.6", 20:9) | 보급형 기기 성능 테스트 |
| P1 | Pixel 7a (6.1", 20:9) | 순정 Android 동작 확인 |
| P2 | iPad Mini (8.3", 3:2) | 태블릿 레터박스 표시 확인 |

**접근성 테스트:**
- 색약 시뮬레이션 (적록색약, 청황색약) → 모든 상태 정보가 형태로도 구분되는지 확인
- 시스템 폰트 크기 1.5배 설정 → UI 깨짐 없는지 확인
- 사운드 OFF 상태 → 모든 피드백이 시각으로 전달되는지 확인
- 한 손(왼손/오른손) 테스트 → 핵심 액션 도달 가능 확인

**성능 테스트:**
- 아이소메트릭 구장 + 캐릭터 8~12명 활동 시 60fps 유지 (보급형 기기 기준)
- 저전력 모드: 캐릭터 정적 이미지 + 배경 애니메이션 중단 → 배터리 소모 50% 감소
- 콜드 스타트 → 게임 플레이 가능까지 3초 이내
...
| 영역 | 가이드라인 |
|------|----------|
| **해상도 처리** | Project Settings > Display > Window에서 기준 해상도 설정 → Stretch Mode: canvas_items, Aspect: expand |
| **세이프 에어리어** | OS.get_safe_area() 활용하여 노치/제스처 바 영역 인셋 적용 |
| **터치 영역** | Control 노드의 minimum_size를 48×48dp로 설정. 시각적 크기와 터치 영역 분리 가능 |
| **스케일링** | 픽셀 아트 텍스처: Filter = Nearest (보간 없음). 정수 배율만 적용 |
| **사이드 메뉴** | AnimationPlayer로 슬라이드 인/아웃. ColorRect로 배경 어둡게. InputEventScreenTouch로 메뉴 밖 탭 감지 |
| **오버레이 패널** | CanvasLayer로 구장 위에 별도 레이어. Panel의 self_modulate alpha로 반투명 처리 |
| **팝업** | PopupPanel 노드 활용. 모달 모드로 배경 입력 차단 |
| **저전력 대응** | OS.get_power_state()로 배터리 상태 감지. 저전력 시 AnimationPlayer 정지 + 정적 스프라이트로 전환 |
